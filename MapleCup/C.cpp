...
int knapsack[14][9001]; // 시간 내 잡을 수 있는 최대 메소
...
int main(void)
{
    ...
    for(int s = 0; s < m; s++) // m개의 캐릭터에 대해 연산 실시
    {
        ...
        fill_n(knapsack[0],14*9001,0); // 초기화
        int_64 dps = character[s];
        cout << dps << '\n';
        for (int i = 1; i <= k; i++) // 보스몹
        { 
            for (int j = 1; j <= 9000; j++) { // 시간
                if (int(ceil(boss[i].first / dps)) > j) // j초 내에 보스를 잡을 수 없다면
                    knapsack[i][j] = knapsack[i - 1][j]; // 전의 노드와 동일
                else // j초 내에 이 보스를 잡을 수 있다면 {1초 전에 벌 수 있었던 최대 메소 vs 현재 보스를 잡는데 걸리는 시간 전의 최대 메소 + 잡아서 번 메소} 비교
                {
                    knapsack[i][j] = max(knapsack[i - 1][j], knapsack[i - 1][j - int(ceil(boss[i].first / dps))] + boss[i].second);
                }
            }
        }
        cout << knapsack[13][9000] << '\n';
        meso += knapsack[13][9000];
    }
    cout << meso << '\n';
    return 0;
}